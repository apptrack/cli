...
Exec.prototype = {
	...
	user : function( domain, options ){
		var apptrack = this.api.auth({key: this.creds.key ,secret: this.creds.secret, app: this.creds.app });
	...
	sub : function( domain, options ){
		// fallbacks
		options = options || {};
		var apptrack = this.api.auth({keyid: this.creds.key ,secret: this.creds.secret, app: this.creds.app });

		if( domain == "*" ){
			// read the domain list instead
			return apptrack.list(function( error, result ) {
		...
		apptrack.select( query, function( error, result ) {
			if( error ) return console.log( error );
			output( result );
		});
