...
var Exec = function( program ){
	...
	// init lib
	this.api = new Apptrack( this.config );
...
Exec.prototype = {
	...
	user: function( id, options ){
		...
		if( options.logs ) params.type = "logs";
	...
	app: function( id, options ){
		...
		if( options.log ) params.type = "log";

		this.get( params );
	},

	log: function( id, options ){
		var params = {};
		if( id ) params.id = id ;

		// read the product collection for the current user: /logs
		params.name = ( options.list ) ? "logs" : "log";
	...
	getToken: function( callback ){
		...
		if( !this.config.email || !this.config.token ) return output("Credentials not available - Please run setup: $ apptrack setup");
		//
